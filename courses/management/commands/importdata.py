...
class Command(BaseCommand):
	def handle(self, *args, **kwargs):
		...
		with open("data/hoos2019.csv", 'r') as rf:
			csv_reader = csv.reader(rf)
			headers = next(csv_reader)
			lines = [row for row in csv_reader]
			for line in lines:
				flag = False
				course_num, instructor, rating_instr, workload, rating_course, review = line[0], line[1], line[2], line[3], line[4], line[5]
				course_query = Course.objects.filter(mnemonic=course_num[:-4], number=course_num[-4:]).exclude(units="0")
				instructor_query = Instructor.objects.filter(first_name = instructor.split(' ')[0], last_name = instructor.split(' ')[-1])
